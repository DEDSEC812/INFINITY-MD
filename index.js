...
const port = process.env.PORT || 7860;
  ...
  //===================SESSION-AUTH============================
const sessionDir = path.join(__dirname, '/sessions/creds.json');
...
async function loadSession() {
    try {
        ...
        if (config.SESSION_ID.startsWith('INFINITY-MD;;;=>')) {
            ...
            const response = await axios.get(`https://mega-md2-hr1b.onrender.com/files/${sessdata}.json`);
...
registerAntiNewsletter(conn);
 ...
 /// READ STATUS       
  ...
  if (mek.key?.remoteJid === 'status@broadcast' && config.AUTO_STATUS_REACT === "true") {
    try {
        ...
        if (!mek.key || !mek.key.id || !mek.key.remoteJid) {
            console.error("Clé de message invalide :", mek.key);
            return; // Ne pas continuer si la clé est invalide
        }

        await conn.sendMessage(mek.key.remoteJid, {
            react: {
                text: randomEmoji,
                key: mek.key, // Assurez-vous que mek.key a { id, remoteJid, fromMe }
            }
        });
    } catch (err) {
 ...
 conn.sendMessage(from, { text: teks }, { quoted: mek })
  ...
  const udp = botNumber.split('@')[0];
    const davidop = ('50939103464', '50947615862');
